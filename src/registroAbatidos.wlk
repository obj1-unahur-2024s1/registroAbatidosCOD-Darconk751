object registroAbatidosCOD {
	...
	method primerValorDeAbatidos() = registro.get(0).first() // "get(0)" es igual a "first()"
	method ultimoValorDeAbatidos() = registro.get(0).last()
	method maximoValorDeAbatidos() = registro.max({datoDelRegistro => datoDelRegistro.get(0)})
	...
	method diasConAbatidosSuperioresA(cantidad) = registro.filter({datoDelRegistro => datoDelRegistro.get(0) > cantidad}).map({datoDelRegistro => datoDelRegistro.get(1)})
	...
	method esCrack() = registro.all({datoDelRegistro => datoDelRegistro})
}